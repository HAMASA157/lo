...
async def sourc(client: Client, message: Message):
    await message.reply_photo(
        ...
        caption=f"""✧ 𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝑻𝒐 𝙎𝙨𝙞𝘼𝙖 𝙈𝘂𝙎𝗶𝘾 ♪\n\n• ᴅᴇᴠᴇʟᴏᴘᴇʀ -› Y_F_G5\n• ᴄʜᴀɴɴᴇʟ ᴍɪʀᴀ -› @UI_IUSIA\n\n**- للتنصيب او للاستفسار تواصل مع مطور السورس**""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                       "مطور السورس", user_id=5847566596)
                ],[
                    InlineKeyboardButton(
                       "تحديثات ميرا", url=f"https://t.me/UI_IUSIA")
...
async def aboutd5ev(client: Client, message: Message):
    usr = await client.get_chat(5847566596)
    name = usr.first_name
    bio = (await client.get_chat(5847566596)).bio
    async for photo in client.iter_profile_photos(5847566596, limit=1):
                    await message.reply_photo(photo.file_id, caption=f"""- 𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓 𝑻𝒐 𝑩𝒐𝒕 𝙎𝙨𝙞𝘼𝙖 ♪ -› Y_F_G5\n\n- 𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓'𝒔 𝑩𝒊𝒐 -› {bio}""", 
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
